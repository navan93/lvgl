...
#define DISPLAY_DC      2       //  LCD_DC (P0.02): Clock/data pin (CD)
#define DISPLAY_RST     31      //  LCD_RESET (P0.31): Display reset
#define DISPLAY_HIGH    30      //  LCD_BACKLIGHT (P0.30): Backlight (active high)
...
#define FLASH_DEVICE 0  //  0 for Internal Flash ROM, 1 for External SPI Flash
...
static struct hal_spi_settings spi_settings = {
    ...
    .baudrate   = MYNEWT_VAL(SPI_BAUDRATE)
};
...
int pinetime_lvgl_mynewt_init_display(void) {
    ...
    (void)hal_spi_disable(DISPLAY_SPI);
    rc = hal_spi_config(DISPLAY_SPI, &spi_settings);
    if (rc) {
        return (rc);
    }
    hal_spi_set_txrx_cb(DISPLAY_SPI, NULL, NULL);
    hal_spi_enable(DISPLAY_SPI);
